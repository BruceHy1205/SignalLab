...
from to_package import rdagent_result_to_package  # noqa: E402
...
def _run_stub(guard: BudgetGuard, reporter: Reporter) -> tuple[str, float]:
    ...
    while True:
        ...
        best_ic = round(best_ic + 0.004, 4)
        reporter.event(
            "loop_done",
            loop=guard.loops,
            best_ic=best_ic,
            llm_cost_usd=guard.llm_cost_usd,
            message=f"rdagent stub loop {guard.loops}",
        )
        ...
        "factors": [
            {
                ...
                "description": "1日反转(rdagent stub)",
                "hypothesis": "fin_factor stub hypothesis",
            }
        ],
        "metrics": {
            "ic": claimed,
...
def main() -> int:
    ...
    try:
        if backend == "real":
            ...
            reporter.event("started", message="rdagent stub runner started")
    ...
    data = rdagent_result_to_package(
